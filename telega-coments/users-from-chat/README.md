...
```bash
users-from-coments/venv/bin/python users-from-chat/collect_chat_users.py --account chat
```

From `/home/garg/openAI`:

```bash
telega-coments/users-from-coments/venv/bin/python telega-coments/users-from-chat/collect_chat_users.py --account chat
```
...
```bash
users-from-coments/venv/bin/python users-from-chat/collect_chat_users.py --account sender --limit 20
```

Change the time window:

```bash
users-from-coments/venv/bin/python users-from-chat/collect_chat_users.py --account chat --months 3
```
