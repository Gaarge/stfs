...
```bash
users-from-coments/venv/bin/python users-from-rusprofile/fill_contacts_from_rusprofile.py
```

Quick test:

```bash
users-from-coments/venv/bin/python users-from-rusprofile/fill_contacts_from_rusprofile.py --limit 5 --dry-run
```
...
```bash
users-from-coments/venv/bin/python users-from-rusprofile/phone_to_telegram_users.py --account search --limit 5
```
...
```text
users-from-rusprofile/sessions/search.session
```
